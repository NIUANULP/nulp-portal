const express = require("express"),
  ...
  oneDayMS = 86400000,
  pathMap = {},
  cdnIndexFileExist = fs.existsSync(
    path.join(__dirname, "../dist", "index_cdn.ejs")
  ),
...
module.exports = (app, keycloak) => {
  ...
  app.get(["*.js", "*.css"], (req, res, next) => {
    ...
    res.setHeader(
      "Expires",
      new Date(Date.now() + oneDayMS * 30).toUTCString()
    );
    if (
      req.get("Accept-Encoding") &&
      req.get("Accept-Encoding").includes("br")
    ) {
      // send br files
      if (
        !setZipConfig(req, res, "br", "br") &&
  ...
  app.use(
    "/sunbird-plugins",
    express.static(path.join(__dirname, "../sunbird-plugins"))
  );

  app.use(
    "/tenant",
    express.static(path.join(__dirname, "../tenant"), { index: false })
  );

  if (envHelper.DEFAULT_CHANNEL) {
    app.use(
      express.static(
        path.join(__dirname, "../tenant", envHelper.DEFAULT_CHANNEL)
      )
    );
  }
  ...
  app.all(["*/dial/:dialCode", "/dial/:dialCode"], (req, res) => {
    if (_.get(req, "query.channel")) {
      res.redirect(
        `/${_.get(req, "query.channel")}/get/dial/${
          req.params.dialCode
        }?source=scan`
      );
    } else {
...
function getLocals(req) {
  ...
  if (req.includeUserDetail) {
    ...
    locals.sessionId =
      _.get(req, "sessionID") && _.get(req, "session.userId")
        ? req.sessionID
        : null;
  ...
  locals.extContWhitelistedDomains =
    envHelper.sunbird_extcont_whitelisted_domains;
  ...
  locals.offlineDesktopAppVersion =
    envHelper.sunbird_portal_offline_app_version;
  locals.offlineDesktopAppReleaseDate =
    envHelper.sunbird_portal_offline_app_release_date;
  (locals.offlineDesktopAppSupportedLanguage =
    envHelper.sunbird_portal_offline_supported_languages),
  ...
  (locals.logFingerprintDetails = envHelper.LOG_FINGERPRINT_DETAILS),
    (locals.slugForProminentFilter =
      envHelper.sunbird_portal_slugForProminentFilter),
    (locals.deviceId = "");
  ...
  (locals.sunbirdQuestionSetChildrenLimit =
    envHelper.sunbird_questionset_children_limit),
    (locals.sunbirdCollectionChildrenLimit =
      envHelper.sunbird_collection_children_limit),
...
const indexPage = (loggedInRoute) => {
  return async (req, res) => {
    if (envHelper.DEFAULT_CHANNEL && req.path === "/") {
      ...
      if (!loggedInRoute) {
        // for public route, if user token is valid then send user details
        await proxyUtils
          .validateUserToken(req, res)
          .catch((err) => (req.includeUserDetail = false));
      }
...
const renderDefaultIndexPage = (req, res) => {
  ...
  if (
    (req.path == "/get" || req.path == `/${req.params.slug}/get`) &&
    ...
    if (
      envHelper.PORTAL_CDN_URL &&
      cdnIndexFileExist &&
      req.cookies.cdnFailed !== "yes"
    ) {
      // assume cdn works and send cdn ejs file
      res.locals.cdnWorking = "yes";
      res.render(path.join(__dirname, "../dist", "index_cdn.ejs"));
    } else {
      // load local file if cdn fails or cdn is not enabled
...
const renderTenantPage = (req, res) => {
  const tenantName =
    _.lowerCase(req.params.tenantName) || envHelper.DEFAULT_CHANNEL;
  ...
  if (envHelper.TENANT_CDN_URL) {
    request(
      `${envHelper.TENANT_CDN_URL}/${tenantName}/index.html`,
      (error, response, body) => {
...
const loadTenantFromLocal = (req, res) => {
  ...
  if (
    tenantName &&
    fs.existsSync(path.join(__dirname, "./../tenant", tenantName, "index.html"))
  ) {
...
const redirectTologgedInPage = (req, res) => {
  let redirectRoutes = {
    "/explore": "/resources",
    "/explore/1": "/search/Library/1",
    "/explore-course": "/learn",
    "/explore-course/1": "/search/Courses/1",
    "/guest-profile": "/profile",
    "/explore-groups": "/my-groups",
  };
  if (req.params.slug) {
    redirectRoutes = {
  ...
  if (
    _.get(req, "query.redirect_uri") &&
    _.get(req, "query.redirect_uri").includes(VDNURL)
  ) {
  ...
  if (_.get(req, "sessionID") && _.get(req, "session.userId")) {
    if (_.get(redirectRoutes, req.originalUrl)) {
      ...
      const urlWithOutSlug = req.params.slug
        ? req.originalUrl.replace("/" + req.params.slug, "")
        : req.originalUrl;
      const courseUrl = urlWithOutSlug.includes("/explore-course/course/");
      if (courseUrl) {
        return res.redirect(
          urlWithOutSlug.replace("/explore-course/course/", "/learn/course/")
        );
      }
      ...
      if (_.get(redirectRoutes, `/${req.originalUrl.split("/")[1]}`)) {
        ...
        _.includes(_.get(req, "originalUrl"), "get") ||
        _.includes(_.get(req, "originalUrl"), "dial")
      ) {
        req.includeUserDetail = true;
        renderDefaultIndexPage(req, res);
      } else {
...
const playContent = (req, res) => {
  if (
    req.path.includes("/play/quiz") &&
    fs.existsSync(path.join(__dirname, "../tenant/quiz/", "index.html"))
  ) {
...
const MLContent = (req, res) => {
  if (
    req.path.includes("/manage-learn/") &&
    fs.existsSync(path.join(__dirname, "../tenant/manage-learn/", "index.html"))
  ) {
...
const redirectToLogin = (req, res) => {
  ...
  const query = `?client_id=portal&state=3c9a2d1b-ede9-4e6d-a496-068a490172ee&redirect_uri=https://${req.get(
    "host"
  )}/${redirectUrl}&scope=openid&version=${
    CONSTANTS.KEYCLOAK.VERSION
  }&response_type=code&error_message=${req.query.error_message}`;
