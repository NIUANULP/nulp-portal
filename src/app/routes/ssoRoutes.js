...
const getErrorMessage = (error, errorType) => {
  if(_.get(error, 'params.err') === 'USER_ACCOUNT_BLOCKED') {
    ...
    return 'Your account could not be signed in to NULP due to invalid credentials provided. Please try again with valid credentials.';
    ...
    return 'Your account could not be signed in to NULP due to technical issue. Please try again after some time';
  }
}
