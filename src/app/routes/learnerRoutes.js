...
const {
  ...
  isDateExpired,
  decodeNChkTime,
} = require("../helpers/utilityService");
  ...
  app.patch(
    ...
    proxy(envHelper.learner_Service_Local_BaseUrl, {
      proxyReqOptDecorator: proxyUtils.decorateRequestHeaders(
        envHelper.learner_Service_Local_BaseUrl
      ),
      ...
      userResDecorator: (proxyRes, proxyResData, req, res) => {
        logger.info({
          msg: "/learner/portal/user/v1/update called upstream url /private/user/v3/update",
        });
        try {
          ...
          logger.error({
            msg: "learner route : userResDecorator json parse error:",
            proxyResData,
          });
          logger.error({
            msg: "learner route : error for /learner/portal/user/v1/update upstram url is /private/user/v3/update ",
            err,
          });
          return proxyUtils.handleSessionExpiry(
            proxyRes,
  ...
  app.all(
    ...
    proxy(learnerURL, {
      ...
      proxyReqPathResolver: function (req) {
        ...
        if (urlParam.indexOf("anonymous") > -1)
          urlParam = urlParam.replace("anonymous/", "");
        ...
        logger.info({
          msg: "/learner/* called - " + req.method + " - " + req.url,
        });

        if (req.originalUrl === " /learner/rc/certificate/v1/search") {
          logger.info({
            msg:
              "/learner/rc/certificate/v1/search called - " +
              req.method +
              " - " +
    ...
    userResDecorator: function (proxyRes, proxyResData, req, res) {
      try {
        ...
        if (
          ...
          return proxyUtils.handleSessionExpiry(proxyRes, data, req, res, data);
      } catch (err) {
        logger.error({
          msg: "learner route : userResDecorator json parse error:",
          proxyResData,
...
function checkForValidUser() {
  return proxy(learnerURL, {
    ...
    proxyReqBodyDecorator: function (bodyContent, srcReq) {
      ...
      if (
        decodedValidator["key"] &&
        (reqEmail === decodedValidator["key"] ||
          reqPhone === decodedValidator["key"])
      ) {
    ...
    proxyReqPathResolver: function (req) {
      return require("url").parse(
        envHelper.LEARNER_URL + req.originalUrl.replace("/learner/", "")
      ).path;
    },
    userResDecorator: function (proxyRes, proxyResData, req, res) {
      try {
...
function proxyObj() {
  return proxy(learnerURL, {
    ...
    proxyReqPathResolver: function (req) {
      let urlParam = req.originalUrl.replace("/learner/", "");
      let query = require("url").parse(req.url).query;
      if (urlParam.indexOf("isUserExists") > -1)
        urlParam = urlParam.replace("isUserExists/", "");
      if (query) {
        return require("url").parse(learnerURL + urlParam + "?" + query).path;
      } else {
        return require("url").parse(learnerURL + urlParam).path;
