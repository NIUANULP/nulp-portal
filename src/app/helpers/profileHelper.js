...
const certificateCount = async (req, res) => {
    ...
    await Promise.all(
      courses.map(async (item) => {
        ...
        const filteredBatchList = getBatchList?.data?.result?.response?.content.filter(batch => batch.cert_templates !== null && Object.keys(batch.cert_templates).length > 0);
...
const courseCount = async (req, res) => {
    ...
    item => new Date(item?.enrolledDate) >= firstDayOfLastMonth && new Date(item?.enrolledDate) < firstDayOfCurrentMonth
    );
