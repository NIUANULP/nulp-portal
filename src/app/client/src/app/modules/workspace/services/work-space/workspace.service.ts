...
export class WorkSpaceService {
  ...
  public readonly questionSetEnabled$: Observable<any> =
    this._questionSetEnabled$
      .asObservable()
  ...
  navigateToContent(content, state) {
    ...
    if (mimeType === "application/vnd.ekstep.content-collection") {
      ...
      this.config.appConfig.WORKSPACE.genericMimeType.includes(mimeType)
    ) {
  ...
  openCollectionEditor(content, state) {
    let navigationParams = [
      "/workspace/content/edit/collection",
      content.identifier,
      content.contentType,
      state,
      content.framework,
    ...
    if (
      (_.toLower(content.contentType) === "course" &&
        _.toLower(content.primaryCategory) === "course") ||
      _.toLower(content.contentType) === "collection" ||
      _.toLower(content.contentType) === "textbook"
    ) {
      navigationParams = [
        "workspace/edit/",
        content.contentType,
  ...
  openContent(content, state) {
    ...
    if (this.config.appConfig.WORKSPACE.states.includes(state)) {
      ...
      if (state === "upForReview") {
        ...
        this.route.navigate([
          "workspace/content/flag/content",
          content.identifier,
        ]);
      } else if (state === "review") {
        this.route.navigate([
          "workspace/content/review/content",
          content.identifier,
        ]);
      } else if (state === "flagreviewer") {
        this.route.navigate([
          "workspace/content/flagreviewer/content",
          content.identifier,
        ]);
      }
    }
  ...
  toggleWarning(type?: string) {
    ...
    if (this.showWarning === "true") {
      this.listener = (event) => {
        ...
        if (event.state) {
          const alertMsg = type
            ? this.resourceService.messages.imsg.m0038 +
              " " +
              type +
              ", " +
              this.resourceService.messages.imsg.m0039
  ...
  getFormData(formInputParams): Observable<ServerResponse> {
    const pageData: any = this.cacheService.get(
      formInputParams.formAction + formInputParams.subType
    );
    if (pageData) {
      ...
      return this.publicDataService.post(channelOptions).pipe(
        map((data) => {
          this.setData(
            data,
            formInputParams.formAction + formInputParams.subType
  ...
  getQuestionSetCreationStatus() {
    ...
    this.getFormData(formInputParams).subscribe(
      (response) => {
        ...
        this._questionSetEnabled$.next({
          err: null,
          questionSetEnablement: displayValue,
        });
      },
      (error) => {
        this._questionSetEnabled$.next({
          err: error,
          questionSetEnablement: false,
        });
        ...
        if (event.state) {
          const alertMsg = type
            ? this.resourceService.messages.imsg.m0038 +
              ...
              type +
              ", " +
              this.resourceService.messages.imsg.m0040
            : this.resourceService.messages.imsg.m0037;
          this.browserBackEvent.emit();
          alert(alertMsg);
          window.location.hash = "no";
