...
@Component({
  selector: 'app-course-content-leaderboard',
  templateUrl: './course-content-leaderboard.component.html',
  styleUrls: ['./course-content-leaderboard.component.scss']
})
export class CourseContentLeaderboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.colsUser = [
      ...
      { field: 'Total_Content', header: 'Total Course', },
    ]
  ...
  getContentData() {
    ...
    const data = {
      ...
      {
        "query": "",
        "filters": { "status": ["Live"], "framework": ["nulp"], "channel": this.rootOrgId, "contentType": ["Course"] },
        "limit": "1000",
  ...
  CreateLeaderBoardChart(ResultCT: any) {
    ...
    this.popupTitle = "List of Top 10 Course Creators in '" + this.titleCity + "'.";
  ...
  L1_Chart(L1_Name: any, L1_Value: any, L1_Filter: any, L1_Colour: any, L1_Title: any) {
    ...
    this.L1_Piechart = new Chart('L1_Canvas_Course_Leadership',
      {
        type: 'bar',
        data:
        {
        ...
        options:
        {
          ...
          scales: {
            xAxes: [{ scaleLabel: { display: true, labelString: 'Course Contributors' } }],
            yAxes: [{ ticks : { min : 0 }, scaleLabel: { display: true, labelString: 'Course Count' }}],
          },
          // plugins: {
          //   labels: [
