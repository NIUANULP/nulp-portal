...
@Component({
  selector: 'app-course-progress-report',
  templateUrl: './course-progress-report.component.html',
  styleUrls: ['./course-progress-report.component.scss']
})
export class CourseProgressReportComponent implements OnInit, OnDestroy { //, AfterViewInit
...
//    this.showLoader = true; // commenting for the time-being
    ...
    combineLatest(
      ...
      .subscribe((results) => {
        ...
        if (this.batchlist.length === 0) {
          ...
          this.populateCourseProgressReportData(isBatchExist);
          ...
          this.populateCourseProgressReportData(this.currentBatch);
        } else {
  ...
  populateCourseProgressReportData(batch: any) {

//    this.showLoader = true; //commenting for the timebeing.
    ...
    // }
    this.courseProgressService.getCourseProgressReportData(option).pipe(
      ...
      .subscribe(
        (apiResponse: ServerResponse) => {
          ...
          this.showLoader = false;
          this.courseProgressReportData = apiResponse.result;
...
*/
  ...
  */
  ngOnInit() {
    this.userProgressList = courseProgressData.result.content;
