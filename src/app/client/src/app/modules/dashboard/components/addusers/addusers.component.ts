...
export class AddusersComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.createUserForm = new FormGroup({
      firstname: new FormControl("", [
        Validators.required,
        Validators.pattern(this._validation.alphanumericspaceRegex),
      ]),
      ...
      emailid: new FormControl("", [
        Validators.required,
        Validators.pattern(this._validation.emailRegex),
      ]),
      phone: new FormControl("", [
        Validators.required,
        Validators.pattern(this._validation.mobileno),
      ]),
    ...
    this.editOrgForm = new FormGroup({
      orgName: new FormControl(this.getOrgName, [
        Validators.required,
        Validators.pattern(this._validation.alphanumericspaceRegex),
      ]),
    ...
    this.selectedItems = [{ id: 1, itemName: "PUBLIC" }];
    this.dropdownList = [{ id: 1, itemName: "PUBLIC" }];
  ...
  getOrgDataUser() {
    ...
    this._httpService.getSuborgData(tempArray).subscribe(
      (res) => {
        res.result.response.content.forEach((element) => {
          this.getAllOrgData.push({
    ...
    this.userSearchService.getUserById(option).subscribe(
      (apiResponse: ServerResponse) => {
        ...
        sessionStorage.setItem(
          "userLoginDataChannel",
          this.userLoginDataChannel
        );
        //Organization tab is visible for system admin and Root Admin
        this.orgDataRole = this.userLoginData.roles.map((obj) => obj.role);
        if (
          this.orgDataRole.includes("ORG_ADMIN") ||
          this.orgDataRole.includes("SYSTEM_ADMINISTRATION")
        ) {
          this.userTab = true;
  ...
  populateUserSearch(userLoginDataChannel, organisationId, systemVar, query?) {
    ...
    this._httpService.userSearch(tempArray).subscribe((res) => {
      ...
      res.result.response.content.forEach((element) => {
        ...
        if (this.orgDatalength == 1) {
          ...
          this.getAllOrgData.forEach((element1) => {
            ...
            if (element.organisations.length > 1) {
              if (
                element1.identifier ==
                  element.organisations[1].organisationId &&
        ...
        this.showUserData.push({
          orgType: this.orgTypeUser,
          userId: element.id,
          uStatus: element.status,
          createdDate: element.createdDate,
          firstName: element.firstName,
          lastName: element.lastName,
          email: element.email,
          phone: element.phone,
          orgLength: element.organisations.length,
          orgName: this.orgNameUser,
          status: this.status,
          userOrglengths: this.userOrgLength,
      ...
      this.showUserData = this.showUserData.sort(
        (a, b) =>
      ...
      this.orgTypelListUserArry = this.orgTypelListUser.filter(
      ...
      this.statusList = this.showUserData
        ...
        .filter((value, index, self) => self.indexOf(value) === index);
      this.statuslistArry = this.statusList.filter(
        (f) => f !== undefined && f !== null
      ) as any;
      ...
      this.orgList = this.showUserData
        ...
        .filter((value, index, self) => self.indexOf(value) === index);
      this.orgListArry = this.orgList.filter(
        (f) => f !== undefined && f !== null
      ) as any;
  ...
  createOrgFormSubmit() {
    {
      ...
      this._httpService.orgSearch(tempArray1).subscribe(
        (res) => {
          ...
          if (this.countOrgRecord > 0) {
            ...
            this.userLoginDataChannel = sessionStorage.getItem(
              "userLoginDataChannel"
            );
            this.rootOrgId = sessionStorage.getItem("rootOrgId");
            if (this.systemVar == "present") {
              //alert('first');
              this.onchangeorgName =
                this.createOrgForm.value["rootOrgName"].split("/");
    ...
    this._httpService.orgSearch(tempArray1).subscribe(
      (res) => {
        ...
        if (this.countOrgRecord > 0) {
          ...
          this._httpService.createRootOrgDetailSave(tempArray).subscribe(
            (res) => {
              ...
              this.requestChangeFormCollectionData(
                res.result["organisationId"]
              );
              this.collectionReviewFormData(res.result["organisationId"]);
              this.collectionResourceFilterFormData(
                res.result["organisationId"]
              );
              ...
              this.requestChangeFormLessonPlamData(
                res.result["organisationId"]
              );
              this.lessonPlanSaveData(res.result["organisationId"]);
              this.lessonPlanResourceFilterFormData(
                res.result["organisationId"]
              );
            ...
            (err) => {
              ...
              this.toasterService.error(
                this.resourceService.messages.emsg.m0005
              );
            }
          );
        }
  ...
  getUserOrganization(checkSystemAdmin: any) {
    ...
    this._httpService.orgSearch(tempArray).subscribe((res) => {
      ...
      this.orgTypelListOrg = this.showOrgData
        .map((item) => item.orgType)
        .filter((value, index, self) => self.indexOf(value) === index);
      this.orgTypeOrgArry = this.orgTypelListOrg.filter(
        (f) => f !== undefined && f !== null
      ) as any;
      ...
      this.channelListOrg = this.showOrgData
        ...
        .filter((value, index, self) => self.indexOf(value) === index);
      this.channelListOrgArry = this.channelListOrg.filter(
        (f) => f !== undefined && f !== null
      ) as any;
      ...
      this.statusListOrg = this.showOrgData
        .map((item) => item.status)
        .filter((value, index, self) => self.indexOf(value) === index);
      this.statuslistArryOrg = this.statusListOrg.filter(
        (f) => f !== undefined && f !== null
      ) as any;
      ...
      this.orgListOrg = this.showOrgData
        .map((item) => item.orgName)
        .filter((value, index, self) => self.indexOf(value) === index);
      this.orgListArryOrg = this.orgListOrg.filter(
        (f) => f !== undefined && f !== null
      ) as any;
  ...
  updatePatch(OrgId: any) {
    let tempArray: any;
    tempArray = { request: { channel: { defaultFramework: "nulp" } } };
    this._httpService.updatePatch(tempArray, OrgId).subscribe(
      (res) => {
        this.addOrgrPopup = false;
      },
      (err) => {
  ...
  contentPublishFormData(rootOrgIdCreate: any) {
    ...
    this._httpService.createForm(tempArray1).subscribe(
      (res) => {
        this.addOrgrPopup = false;
      },
      (err) => {
      ...
      request: {
        ...
        data: {
          ...
          fields: [
            {
              title:
                "Please tick the reasons for requesting changes and provide detailed comments:",
              otherReason:
                "Other Issue(s) (if there are any other issues, tick this and provide details in the comments box)",
              contents: [
                {
                  name: "Appropriateness",
                  checkList: [
  ...
  createPreference(rootOrgIdCreate: string) {
    ...
    this._httpService.createPreference(pref_obj).subscribe(
      (res) => {
        this.addOrgrPopup = false;
      },
      (err) => {
  ...
  editOrgFormSubmit() {
    ...
    this._httpService.updateOrgDetail(tempArray).subscribe(
      (res) => {
        this.editOrgrPopup = false;
        this.addRootOrgrPopup = false;
        this.addOrgrPopup = false;
  ...
  addRole(gridUserId: any) {
    ...
    this.selectedItems = [];
    this.selectedItems = [{ id: 1, itemName: "PUBLIC" }];
    this._httpService.getEditUserById(gridUserId).subscribe(
      (response) => {
        ...
        if (this.roleEditeUserData.organisations.length == 1) {
          ...
          this.addRoleorgId =
            this.roleEditeUserData.organisations[0].organisationId;
        } else if (
          this.roleEditeUserData.organisations.length > 1 &&
          this.roleEditeUserData.rootOrgId !=
            this.roleEditeUserData.organisations[1].organisationId
        ) {
          this.addRolePopup = true;
  ...
  getUserOrgList(userChannel: any) {
    ...
    this._httpService.getOrgDetail(tempArray).subscribe(
      (res) => {
        ...
        this.addorgSelectName =
          this.orgEditeUserData.rootOrg.orgName +
          "/" +
          this.orgEditeUserData.organisations[0].organisationId +
          "/" +
          true;
      },
      (err) => {}
          ...
          if (this.roleEditData[j].role !== "PUBLIC") {
            if (this.roleEditData[j].role == this.dropdownList[i].itemName) {
              this.selectedItems.push({
                ...
                itemName: this.dropdownList[i].itemName,
              });
            }
          }
        }
  ...
  addOrg(gridOrgUserId: any, username) {
    ...
    this.selectedItems = [];
    this.selectedItems = [{ id: 1, itemName: "PUBLIC" }];
    this._httpService.getEditUserById(gridOrgUserId).subscribe(
      (res) => {
        ...
        if (this.orgEditeUserData.organisations.length == 1) {
          ...
          this.roleEditData = this.orgEditeUserData.roles;
          this.addRoleorgId =
            this.orgEditeUserData.organisations[1].organisationId;
  ...
  removeOrg(orgUserId: any) {
    ...
    this._httpService.getEditUserById(this.removeOrgUserId).subscribe(
      (res) => {
        this.genericPopup = true;
        if (res.result.response.organisations.length > 0) {
          if (
            res.result.response.rootOrgId !=
            res.result.response.organisations[1].organisationId
          ) {
            this.removeOrgUserdataAll =
              res.result.response.organisations[1].organisationId;
          } else {
            this.removeOrgUserdataAll =
              res.result.response.organisations[0].organisationId;
          }
        }
      },
      (err) => {}
  ...
  removeOrgSubmit() {
    ...
    this._httpService.removeOrg(removeTempArray).subscribe(
      (res) => {
        this.genericPopup = false;
        location.reload();
      },
  ...
  userBlock(userIds: any) {
    ...
    this._httpService.userBlock(tempArray).subscribe(
      (res) => {
      ...
      (err) => {
        this.sucesErrorPopup = true;
        this.popupMsg =
          "User registration is incomplete and is already inactive..";
      }
    );
  }
  /**
  ...
  userUnBlock(userIds: any) {
    ...
    this._httpService.userUnBlock(tempArray).subscribe(
      (res) => {
        if (res.result.response == "SUCCESS") {
  ...
  editOrgStatusFormSubmit() {
    ...
    this._httpService.updateOrgStatusDetail(tempArray).subscribe(
      (res) => {
  ...
  getRoleName(str: any) {
    ...
    if (this.createUserForm.value["isrootSub"] == "no") {
      ...
      this._httpService.getSuborgData(subOrgList).subscribe(
        (res) => {
  ...
  createUserFormSubmit() {
    ...
    if (
      this.isRootSubCondition == "no" &&
      (this.createUserForm.value["subRootorgname"] == "" ||
        this.createUserForm.value["subRootorgname"] == null)
    ) {
      ...
      // return
      this._httpService.createUserDetailSave(tempArray).subscribe(
        (res) => {
          this.addUserPopup = false;
          if (res.result.response == "SUCCESS") {
            this.createUserId = res.result.userId;
            this.addroleOrganization(
              this.createUserId,
              this.onchangeorgId,
              this.orgRootStatus,
              this.isRootSubCondition
            );
            // location.reload()
          }
        },
  ...
  addroleOrganization(
    ...
    if (isRootSubCondition == "yes" || isRootSubCondition == null) {
      if (orgRootStatus == "false") {
        this._httpService.addroleRootOrganization(tempOrgArray).subscribe(
          (res) => {
            this.sucesErrorPopup = true;
            this.popupMsg = res.result.response;
            //this.populateUserProfile();
  ...
  addRoleFormSubmit() {
    ...
    //return;
    this._httpService.addroleRootOrganization(addRoletempArray).subscribe(
      (res) => {
        this.addRolePopup = false;
        this.sucesErrorPopup = true;
        this.popupMsg = res.result.response;
  ...
  addOrgFormSubmit() {
    ...
    //return;
    this._httpService.addroleSubRootOrganization(addOrgtempArray).subscribe(
      (res) => {
        this.addOrgPopup = false;
        this.sucesErrorPopup = true;
        this.popupMsg = res.result.response;
        if (res.result.response == "SUCCESS") {
          ...
          this._httpService
            .addroleRootOrganization(addOrgtempArray)
            .subscribe();
        }
        // this.populateUserProfile();
      },
