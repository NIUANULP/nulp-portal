...
export class UserReportComponent implements OnInit {
  ...
  getAllData(orgId: any, channel: any) {
    ...
    this.groupedUserDataOrg = [];
    // this.canvas1 = [];
    this._httpService.userSearch(tempArray).subscribe(
      (res) => {
        ...
        this.newStartDateOrg = new Date(
          this.myStartDateOrg[0],
          this.myStartDateOrg[1] - 1,
          this.myStartDateOrg[2]
        );
        this.newTimeStampStartDateOrg = this.newStartDateOrg.getTime();
        this.myEndDateOrg = endDate.split("-");
        this.newEndDateOrg = new Date(
          this.myEndDateOrg[0],
          this.myEndDateOrg[1] - 1,
          this.myEndDateOrg[2]
        );
        ...
        this.showUserDataOrg.forEach((xy) => {
          ...
          if (
            newTimeStampDate >= this.newTimeStampStartDateOrg &&
            newTimeStampDate <= this.newTimeStampEndDateOrg
          ) {
        ...
        this.groupedUserDataOrg = this.showLineChartUserDataOrg.reduce(
          function (rv, x) {
            // grouping of userdata
          ...
          this.subOrg.forEach((element) => {
            ...
            if (this.groupedUserDataOrg[orgNameVal] != undefined) {
              this.organizationData.push({
                ...
                length: this.groupedUserDataOrg[orgNameVal].length,
              });
        ...
        var options = {
          title: {
            display: true,
            text: "User Onboarded in Sub Organization",
            fontSize: 18,
            fontColor: "#111",
          },
          legend: { display: true },
          scales: {
            yAxes: [
              {
                ticks: { min: 0 },
                scaleLabel: { display: true, labelString: "User Count" },
              },
            ],
            xAxes: [
              {
                ticks: { min: 0 },
                scaleLabel: { display: true, labelString: "Organization" },
              },
            ],
            gridLines: { color: "red" },
        ...
        this.Linechart3 = new Chart("canvas13", {
          type: "bar",
          data: data1,
          options: options,
        });
  ...
  dateWiseLineChart() {
    ...
    this._httpService.userSearch(tempArray).subscribe((res) => {
      ...
      res.result.response.content.forEach((element) => {
        ...
        this.getAllOrgChartData.forEach((element1) => {
          ...
          if (this.userOrgLength > 1) {
            if (
              element1.identifier == element.organisations[0].organisationId &&
              element.rootOrgId != element.organisations[0].organisationId
            ) {
              this.orgNameUser = element1.orgName;
            }
          }
        ...
        this.pushLineChartUserData.push({
          userId: element.id,
          uStatus: element.status,
          createdDate: element.createdDate,
          firstName: element.firstName,
      ...
      this.pushLineChartUserData.forEach((xy) => {
        ...
        var newTimeStampDate = newDate.getTime();
        if (
          newTimeStampDate >= this.newTimeStampStartDate &&
          newTimeStampDate <= this.newTimeStampEndDate
        ) {
          this.showLineChartUserData.push(xy);
        }
      ...
      this.dateChartresults = this.showLineChartUserData.reduce(function (
        r,
        b
      ) {
      ...
      var data1 = {
        ...
        datasets: [
          {
            label: "Count",
            data: this.chartrootValue,
            borderColor: this.labelChartRootOrgId,
            borderWidth: this.allChanelName,
            backgroundColor: "#0000FF",
          },
        ],
      };
      var options = {
        title: {
          display: true,
          text: "User Onboarded in Root Organization",
          fontSize: 18,
          fontColor: "#111",
        },
        legend: { display: true },
      ...
      this.Linechart12 = new Chart("canvas12", {
        type: "bar",
        data: data1,
        options: options,
      });
  ...
  showData(evt: any) {
    ...
    if (activePoint.length > 0) {
      ...
      var value =
        this.Linechart12.data.datasets[clickedDatasetIndex].data[
          clickedElementindex
        ];
  ...
  showData3(evt: any) {
    ...
    if (activePoint.length > 0) {
      ...
      var value =
        this.Linechart3.data.datasets[clickedDatasetIndex].data[
          clickedElementindex
        ];
